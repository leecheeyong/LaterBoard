<template>
  <div class="min-h-screen bg-gradient-to-br from-primary-50 via-accent-50 to-indigo-100 flex flex-col">
    <header class="bg-white/80 backdrop-blur-md shadow-lg border-b border-gray-200 sticky top-0 z-30">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-6">
          <div class="flex items-center gap-3">
            <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-primary-100 text-primary-600 shadow-md">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 6v6l4 2"/></svg>
            </span>
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">Later<span class="text-indigo-600">Board</span></h1>
          </div>
          <div class="flex items-center space-x-4">
            <span class="text-base text-gray-700 font-medium">👋 Welcome, {{ user?.email }}</span>
            <button @click="handleLogout" class="btn-secondary text-base rounded-lg shadow-md px-4 py-2">Sign Out</button>
          </div>
        </div>
      </div>
    </header>

    <div v-if="randomCard" class="bg-gradient-to-r from-indigo-50 via-primary-50 to-accent-100 border-b border-indigo-200">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-base text-indigo-800 font-semibold">Idea of the Day</p>
            <p class="text-xl text-indigo-700 font-bold">{{ randomCard.title }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <main class="flex-1">
            ...
            <div class="flex-1 space-y-4">
              ...
              >
                <div class="flex justify-between items-start mb-2">
                  <h4 class="font-semibold text-gray-900 text-lg">{{ card.title }}</h4>
              ...
              <div v-if="getStackCards(stack.id).length === 0" class="text-gray-300 italic text-base py-4 text-center">
      ...
      <div v-if="showAddCardModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div class="bg-white/95 backdrop-blur-2xl rounded-3xl p-10 w-full max-w-lg mx-4 shadow-2xl border-2 border-primary-200">
          <h3 class="text-3xl font-extrabold mb-8 text-primary-700 text-center">Add New Idea</h3>
          <form @submit.prevent="addCard" class="space-y-8">
            ...
            <div class="flex space-x-4 pt-2">
              <button type="submit" class="flex-1 bg-gradient-to-r from-primary-400 to-primary-600 text-gray-900 rounded-xl py-4 text-lg font-bold shadow-lg hover:from-primary-500 hover:to-primary-700 transition">Add Idea</button>
              <button type="button" @click="closeAddCardModal" class="flex-1 btn-secondary rounded-xl py-4 text-lg font-bold">Cancel</button>
            </div>
          </form>
        </div>
      </div>

      <div v-if="showCardModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
        <div class="bg-white/95 backdrop-blur-2xl rounded-3xl p-10 w-full max-w-lg mx-4 shadow-2xl border-2 border-primary-200">
          <h3 class="text-3xl font-extrabold mb-8 text-primary-700 text-center">Edit Idea</h3>
          <form @submit.prevent="updateCard" class="space-y-8">
            ...
            <div class="flex space-x-4 pt-2">
              <button type="submit" class="flex-1 bg-gradient-to-r from-primary-400 to-primary-600 text-gray-900 rounded-xl py-4 text-lg font-bold shadow-lg hover:from-primary-500 hover:to-primary-700 transition">Save Changes</button>
              <button type="button" @click="deleteCard" class="flex-1 bg-red-600 hover:bg-red-700 text-white rounded-xl py-4 text-lg font-bold shadow-lg transition">Delete</button>
              <button type="button" @click="closeCardModal" class="flex-1 btn-secondary rounded-xl py-4 text-lg font-bold">Cancel</button>
            </div>
          </form>
        </div>
      </div>
...
watch(() => user.value && user.value.uid, (val) => {
  if (val) {
    subscribeUserCollections();
  }
});
